...
public class SwaggerConfig implements WebMvcConfigurer {
	...
	public Docket api() {
		return new Docket(DocumentationType.SWAGGER_2).select()
				.apis(RequestHandlerSelectors.basePackage("sk.mirri.ootsevidencefinder.controllers"))
	...
	private ApiInfo apiInfoMetaData() {
		...
		return new ApiInfoBuilder().title("OOTS EVIDENCE FINDER").description("Evidence finder for OOTS PoC")
				...
				// "dev-team@gmail.com"))
				// .license("Apache
				// 2.0").licenseUrl("http://www.apache.org/licenses/LICENSE-2.0.html").version("1.0.0")
				.build();
	}
